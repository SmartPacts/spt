...
#   "Awards accumulate ... selling does not forfeit them"  -> folded into the never-expire row
    ...
    ('P-157', 'A multi-signature founder can spend the tokens they receive, not merely hold them.', 'A multi-signature founder can spend the tokens they', 'PROVEN', 'TRACKED', 'smartpacts-founder-address.repl', 'SPENDS its tranche with 2 of 3 keys'),
    ...
    ('P-090', 'The admin can never send out more treasury or market-making tokens than have vested on the fixed calendar.', '**Treasury and market-making tokens:** the *calendar* is equally fixed — no more can ever be',                                                                        'PROVEN',         'TRACKED', 'smartpacts-tranches.repl',    'SPT disbursement exceeds the vested available amount'),
    ...
    ('P-076', 'The reserve accounts the contract owns can never vote.',                                    'account is one the contract keeps out of voting and awards — the four reserves it owns',                                                                     'PROVEN',         'TRACKED', 'smartpacts-governance.repl',  'excluded reserve cannot vote'),
    ('P-058', 'Voting can never open less than 48 hours after the announcement actually lands on that chain.', 'Voting must open **at least 48 hours after announcing**',                                                                          'PROVEN',         'TRACKED', 'smartpacts-proposal-lifecycle.repl',      'SPT voting must open at least the minimum review gap after this announcement'),
    ('P-036', 'transfer only reaches an account that already exists on this chain.',                       'Sends tokens to an account that already exists on this chain',                                                  'PROVEN',         'TRACKED', 'smartpacts-promises.repl',    'SPT_accounts for key: no-such-account'),
    ('P-021', 'The unlock clock starts at the moment the contract is set up.',                             'The clock starts the moment the contract is set up',                                                                        'PROVEN',         'TRACKED', 'smartpacts-tranches.repl',    '2027-06-01T00:00:00Z'),
    ...
    ('P-134', 'Treasury tokens the admin sends to someone become ordinary tokens that vote.',              'Once sent, they are ordinary',                                                                   'PROVEN',         'OWNER',   'smartpacts-tranches.repl',    'Am.2: a disbursement target votes like any holder'),
    ...
    ('P-068', "A round's effective date must be at least 12 hours in the future.",                         'The effective date must be at least 12 hours away',                                                                          'PROVEN',         'TRACKED', 'smartpacts-record-date.repl',      'must leave at least two retraction leads of runway'),
    ...
    ('P-069', 'A declared round can be cancelled only while more than 6 hours remain before it takes effect.', 'only up to 6 hours before it takes effect',                                                                      'PROVEN',         'TRACKED', 'smartpacts-account-and-round-safety.repl',     'too close to taking effect'),
    ...
    ('P-104', 'The price cannot be changed while the sale is open.',                                       'Changes the price — **refused while the sale is open**',                                                        'PROVEN',         'TRACKED', 'SPT-launch.repl',      'SPT price is locked while the sale is active'),
    ...
    ('P-084', 'A disbursement destination must already exist as an SPT account — the contract never creates one for you.', 'must already exist as an SPT account',                                                                         'PROVEN',         'TRACKED', 'smartpacts-tranches.repl',    'SPT_accounts for key: k:b2b2'),
    ('P-112', 'Setting up the other 19 chains issues no tokens.',                                          'No tokens are issued',                                                                          'BUILT-UNPINNED', 'OWNER',   '',                            ''),
    ('P-120', 'The four buckets — 20,000 sale, 10,000 founders, 55,000 treasury, 15,000 liquidity — add up to the whole supply, with nothing left over.', 'all 100,000 tokens to the four reserves',                                                                        'PROVEN',         'TRACKED', 'smartpacts-tranches.repl',    'reserves sum to full 100k with launch 20k'),
    ...
    ('P-122', 'The contract is deployed on Kadena mainnet; the namespace is live on mainnet on all 20 chains and that part is permanent.', '**The contract is deployed on Kadena mainnet**',                    'PROCEDURE',      'OWNER',   '',                            ''),
    ('P-123', 'Nothing goes on a chain until that chain has been checked, at the time and never assumed.', '**Nothing goes on a chain until',                                                                       'PROCEDURE',      'OWNER',   '',                            ''),
