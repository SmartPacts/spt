...
(if (read-msg 'upgrade)
  ;; 🔴 Fail-closed probe, not a status line — see the matching note in SPT's footer.
  ;; A frozen artifact sent as upgrade:true to a chain that never received
  ;; the fresh deploy would otherwise install with no tables and be unrepairable forever.
