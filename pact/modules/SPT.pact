...
(if (read-msg 'upgrade)
  ;; 🔴 THIS READ IS THE WHOLE POINT. It is not a status line — it is a
  ;; fail-closed probe. Sending the FROZEN artifact as upgrade:true to a chain that never got the
  ;; fresh deploy installs it CLEANLY: Pact does not evaluate GOVERNANCE on a fresh install, so the
  ;; frozen module lands with NO TABLES and GOVERNANCE then refuses every repair forever — one
  ;; operator keystroke, one unrecoverable chain, and our own post-freeze verification reports it as
  ;; correct (identical hash, identical stored code). A table read aborts that install at the door.
  ;; BOUNDARY, stated with the decision: this constrains ONLY the upgrade branch of this footer. It
  ;; says nothing about init ordering, nothing about the fresh branch, and nothing about any runtime
  ;; path. ACCEPTED FALSE POSITIVE: it also refuses a legitimate upgrade of a chain that is deployed
  ;; but not yet initialized. That is deliberate — a loud retryable refusal beats a silent permanent
  ;; brick, and the remedy is one command the operator already holds (init, then upgrade).
  ;; 🔴 DO NOT "fix" that false positive with a row-tolerant probe defun: a new defun lives INSIDE
  ;; the module and MOVES THE HASH, reopening the audit for a convenience. And do NOT add a second
  ;; top-level line beginning "(if (read-msg 'upgrade)" — run-tests.sh's frozen-fixture generator
  ;; truncates on that anchor and has eaten a whole module before.
  [ (SPT.get-round-count)
    "upgrade: this version adds no new tables" ]
  [ (create-table accounts)
    (create-table snapshots)        ; record-date generations (sealed float per gen)
    (create-table state)
    ...
    (create-table award-rounds)  ; declared award rounds (global rpt source)
    (create-table round-count)      ; award-round counter
  ])
